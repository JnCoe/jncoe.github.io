...
function contentDiv() {
  ...
  var docWidth = +$('.container-card').width();
...
$('.filters a').click(function (e) {
  ...
  if (filter == "all") {
    $('.container-card').hide().html(articles).fadeIn(1000).append('<div class="content"></div>');
  } else {
    filter = '.' + filter;
    filter_articles = $(articles).filter(filter);
    $('.container-card').hide().html(filter_articles).fadeIn(1000).append('<div class="content"></div>');
  }
...
// ARTICLE CLICK
$('.container-card').on('click', 'article', function (e) {
  e.preventDefault();
